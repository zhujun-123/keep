<template>
	<view class="user-chat-item">
		<view class="user-chat-time u-f-ajc">{{(item.gsttime)}}</view>
		<view class="user-chat-list u-f" :class="{'user-chat-me':item.isme}">
			<image v-if="!item.isme" :src="item.userpic" mode="widthFix" lazy-load />
			<view class="user-chat-list-body">
				<!-- 文字 -->
				<text v-if="item.type=='text'">{{item.data}}</text>
				<!-- 图片 -->
				<image v-if="item.type=='img'" :src="item.data" mode="widthFix" lazy-load />
			</view>
			<image v-if="item.isme" :src="item.userpic" mode="widthFix" lazy-load />
		</view>
	</view>
